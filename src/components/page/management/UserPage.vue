<template>
  <div class="user">
    <el-card v-loading="loading">
      <div class="crud-user">
        <el-button size="small" type="success" @click="addUserDialog = true"
          >添加用户</el-button
        >
        <el-button size="small" type="danger" @click="deleteBatchUserClick"
          >删除用户</el-button
        >
    ...
    <el-dialog title="用户信息" :visible.sync="userDetailDialog" width="650px">
      ...
      >
        ...
        <el-form-item label="附加权限">
          <el-select
            style="width: 195px"
            v-model="userAddPermissions"
            multiple
            placeholder="选择附加权限"
          >
            <el-option
              v-for="item in allUserAddPermissions"
              :key="item.name"
              :label="item.name"
              :value="item.name"
            >
...
import { listAllAddPermissions } from "@/api/permission";
  ...
  methods: {
    ...
    editClick(index, row) {
      ...
      listAllAddPermissionsById(this.userDetail.id).then(response => {
        ...
        let content = response.content;
        for (let i = 0; i < content.length; i++) {
          this.userAddPermissions.push(content[i].name);
        }
        this.userAddPermissionsBack = JSON.stringify(this.userAddPermissions);
      });
